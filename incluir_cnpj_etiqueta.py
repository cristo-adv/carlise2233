# -*- coding: utf-8 -*-
# Inclui o CNPJ da campanha na etiqueta de Kit/Envio de Material (remetente + declaracao de transporte).
import os, time, shutil

BASE = os.path.dirname(os.path.abspath(__file__))
ETQ = os.path.join(BASE, "templates", "impressao_etiquetas_kits.html")

# ---------- 0) BACKUP ----------
ts = time.strftime("%Y%m%d_%H%M%S")
shutil.copy(ETQ, ETQ + ".bak_" + ts)
print("[0/2] Backup: impressao_etiquetas_kits.html.bak_" + ts)
...
<style>
  * { box-sizing: border-box; }
  ...
  .etiqueta { border: 1px solid #999; border-radius: 3px; padding: 3mm 4mm; min-height: 54mm; break-inside: avoid; page-break-inside: avoid; background: #fff; font-size: 11px; }
  ...
  .cnpj-label { font-size: 9px; color: #333; margin-top: 2px; }
...
<body class="fmt-3">
{% macro fmt_cnpj(v) %}{% if v and (v|string)|length == 14 %}{{ v[0:2] }}.{{ v[2:5] }}.{{ v[5:8] }}/{{ v[8:12] }}-{{ v[12:14] }}{% else %}{{ v or '' }}{% endif %}{% endmacro %}
  <div class="no-print">
    <a class="btn btn-cinza" href="javascript:history.back()">&larr; Voltar</a>
  ...
  <div class="grade">
    ...
    <div class="etiqueta">
      ...
      <div class="bloco">
        ...
        {% if cand and cand.cnpj %}<div class="cnpj-label"><b>CNPJ da campanha:</b> {{ fmt_cnpj(cand.cnpj) }}</div>{% endif %}
        {% if cand and cand.comite_endereco %}<div>{{ cand.comite_endereco }}{% if cand.comite_numero %}, {{ cand.comite_numero }}{% endif %}{% if cand.comite_bairro %} &middot; {{ cand.comite_bairro }}{% endif %}{% if cand.comite_cidade %} &middot; {{ cand.comite_cidade }}{% endif %}{% if cand.comite_uf %} / {{ cand.comite_uf }}{% endif %}{% if cand.comite_cep %} &middot; CEP {{ cand.comite_cep }}{% endif %}</div>{% endif %}
      ...
      <div class="dec">
        ...
        Data: {{ data_hoje }}</div>
        {% endif %}
        {% if cand and cand.cnpj %}<div class="cnpj-label">CNPJ da campanha (remetente): {{ fmt_cnpj(cand.cnpj) }}</div>{% endif %}
...
with open(ETQ, "w", encoding="utf-8") as f:
    f.write(TEMPLATE)

if "CNPJ da campanha" in TEMPLATE and "fmt_cnpj" in TEMPLATE:
    print("[1/2] Etiqueta de kit regravada com o CNPJ da campanha no Remetente e na Declaracao de Transporte.")
else:
    print("[1/2] ATENCAO: verifique o conteudo gerado.")

print("[2/2] Pronto! Reinicie o servidor e gere uma etiqueta de kit para conferir (aba Kit -> Gerar Etiqueta).")
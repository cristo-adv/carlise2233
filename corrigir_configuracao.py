# -*- coding: utf-8 -*-
# Corrige a tela CONFIGURACAO: caracteres reais, mascaras (CNPJ/CPF/CEP/tel) e busca de endereco pelo CEP (ViaCEP).
...
for p in (APP, CFG, CFG_FORM):
    if os.path.exists(p):
        shutil.copy(p, p + ".bak_" + ts)
print("[0/4] Backup criado: *.bak_" + ts)

# ---------- 1) COLUNAS DO COMITE NO BANCO (se ainda nao existirem) ----------
...
print("[1/4] Colunas do comite garantidas no banco (cep, numero, bairro, cidade, uf).")

# ---------- 2) CAMPOS DO COMITE NA LISTA DO app.py ----------
app_txt = ler(APP)
marcador = '"comite_endereco","comite_telefone"'
subst = '"comite_endereco","comite_telefone","comite_cep","comite_numero","comite_bairro","comite_cidade","comite_uf"'
if marcador in app_txt:
    app_txt = app_txt.replace(marcador, subst, 1)
    gravar(APP, app_txt)
    print("[2/4] Lista de campos do candidato atualizada (novos campos do comite).")
elif '"comite_endereco", "comite_telefone"' in app_txt:
    app_txt = app_txt.replace('"comite_endereco", "comite_telefone"',
                              '"comite_endereco", "comite_telefone", "comite_cep", "comite_numero", "comite_bairro", "comite_cidade", "comite_uf"', 1)
    gravar(APP, app_txt)
    print("[2/4] Lista de campos atualizada (variante com espacos).")
else:
    print("[!] NAO encontrei os campos do comite na lista do app.py. Nada foi gravado para evitar quebra.")
    raise SystemExit

# ---------- 3) FORMULARIO (configuracao_form.html) com mascaras + busca por CEP ----------
os.makedirs(TPL, exist_ok=True)
gravar(CFG_FORM, """{% extends "base.html" %}
{% block conteudo %}
<div style="padding:10px 0">
<h1>{{ 'Editar candidato' if cand else 'Novo candidato' }}</h1>
<p style="color:#666">Preencha os dados. CNPJ, CPF, CEP e telefones têm m&#225;scara autom&#225;tica. No endere&#231;o do comit&#234;, digite o CEP e clique em &quot;Buscar CEP&quot;: ele carrega tudo (como no cadastro de pessoas).</p>
...
function pega(nome){ var el=document.querySelector('[name="'+nome+'"]'); return el; }
function buscarCep(){
...
{% endblock %}
""")
print("[3/4] Formulario regravado com mascaras e busca por CEP (ViaCEP).")

# ---------- 4) TELA (configuracao.html) com caracteres reais e formatacao ----------
...
{% endblock %}
""")
print("[4/4] Tela de configuracao regravada (caracteres reais + formatacao de CNPJ/CPF/CEP/telefone).")
...
if r.returncode == 0:
    ...
    print(r.stderr)
print("\nConcluido! Reinicie o servidor e abra /configuracao -> Editar candidato para ver as mascaras e a busca por CEP.")